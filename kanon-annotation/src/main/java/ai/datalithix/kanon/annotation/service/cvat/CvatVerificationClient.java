...
public class CvatVerificationClient implements AnnotationProviderVerificationClient {
    private final HttpClient httpClient;

    public CvatVerificationClient(HttpClient httpClient) {
        this.httpClient = httpClient;
    }

    @Override
    public ExternalAnnotationProviderType providerType() {
        return ExternalAnnotationProviderType.CVAT;
    }
    ...
    public ProviderVerificationResult verify(ExternalAnnotationNode node, String resolvedSecret) {
        ...
        try {
            URI endpoint = URI.create(normalizeBaseUrl(node.baseUrl()) + "/api/users/self");
            ...
            if (response.statusCode() >= 200 && response.statusCode() < 300) {
                String version = response.headers().firstValue("X-Cvat-Version").orElse("unknown");
                return new ProviderVerificationResult(
